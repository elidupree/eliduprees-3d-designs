...
plate_depth = spring_wire_thickness + spring_wire_extra_depth + 1.2
lots = 100
...
def top_plate():
    ...
    export("3d_printer_foot_top_plate.stl", "3d_printer_foot_top_plate_2.stl")
    preview(result)

@run_if_changed
def bottom_plate():
    chunk = Face(Circle(Axes(Origin, Up), 25)).extrude(Up*plate_depth)
